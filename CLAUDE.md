...
| `internal/serve` | Serves `POST /v1/chat/completions` and `GET /v1/models`, so every tool that speaks OpenAI reaches the router: policy, fallback, per-domain calibration and spend logging apply exactly as they do to `hyctl dispatch`. The client's `model` field is the **routing instruction**, which turns any OpenAI client's model picker into Hydra's routing UI (`hydra`, `hydra/<enum>`, `hydra/t<N>`, or a head id), and `/v1/models` advertises all of them. `local` routes but records **no** enum, being an alias for GRUNT rather than an enum of its own, or a cost row would name a routing key that does not exist (#832). The whole conversation is flattened into the prompt the router classifies rather than only the last turn: an agent loop's earlier turns carry tool results, which for a code-review client are the user's own source, and both the ledger and the egress gate read that prompt. Binds `127.0.0.1` and **refuses** an off-loopback bind with no token rather than warning: #923 added a check that reports when another local model server answers off-loopback, because Ollama ships with no authentication and says nothing when bound to `0.0.0.0`, and Hydra serving that way by default would make that check describe a risk Hydra itself created. An unparseable address reads as exposed, the safe reading of something unverifiable. `stream: true` answers SSE, and the fallback chain is what that costs: a head failing before it produces anything is invisible, but bytes on the wire cannot be retracted, so a second head's output would arrive appended to the first's and read as one answer. The stream is committed to the head that opened it, and a later delta from another head ends it and cancels the run rather than spending on an answer this request can no longer use. Response headers are held back until that first delta, so a routing mistake or an unreachable head still answers with a status a client can branch on instead of a 200 hiding an error. A head that cannot stream needs no second path, since `executor.Stream` delivers its whole output as one delta, and neither does a cache hit, which runs no head and so emits none at all and is sent whole. Tool calls are folded by the router before it returns, so they are emitted complete before the finish chunk rather than fragment by fragment. `stream_options.include_usage` adds the trailing usage chunk and nothing adds it unasked, which is what the spec says. Both `max_tokens` and `max_completion_tokens` are read because the latter is what open-code-review actually sends. The package deliberately does not import `dispatch`; `cmd/hydra`'s `serveRouter` is the seam, as with `internal/workflow` and `internal/vet`. Drives `hyctl serve`. |
