...
| `internal/awsauth` | AWS credential resolution, in the order AWS's own tools use: the environment, then `$AWS_PROFILE` or `default` in `~/.aws/credentials` and `~/.aws/config`. Its own package because `internal/executor`, which signs, and `internal/provider/env`, which decides whether to list the head at all, have to agree on what configured means, and executor cannot be imported from provider. A credential is taken **whole**: pairing an environment key with a file secret signs as neither identity and fails as a signature error rather than as the misconfiguration it is. A profile whose credential must be fetched (SSO, an assumed role, `credential_process`) yields nothing, since Hydra performs no token exchange, and listing a head for it would promise what no dispatch can deliver. |
