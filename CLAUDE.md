...
| `internal/cost` | Reads `cost.jsonl`, produces spend summaries. `DayOf` is the single answer to which day a row belongs to, and it is the **reader's** day: rows are stamped in UTC, which keeps the log portable, but four sites bucketed on that UTC date while the cockpit's run list already used the local one, so at 00:30 in IST the report was headed with yesterday's date, and the cockpit's own today-cost panel, comparing a local date against a UTC prefix, showed no spend minutes after spending (#916). |
