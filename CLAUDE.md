...
cmd/hyverify/           ← A second binary, the standalone verifier: run the command that judges
...
internal/serve/         ← OpenAI-compatible endpoint in front of the router, so any tool that
                          speaks OpenAI routes through Hydra. `hyctl serve`.
...
internal/verify/        ← Which command judges this work: the repo's own suite, or the workspace
                          validator for the file's language. Shared by the TUI and `hyctl edit`.
...
internal/classify/      ← What the verified corpus says about work like this: the k nearest
                          examples by embedding, each enum's measured pass rate, and a
                          leave-one-out evaluation of whether that predicts anything at all.
...
registry/               ← Routing data, compiled into the binary via `go:embed` (registry.go)
  ...
  signals.yaml          ← Routing signals and rules. Named, typed facts about a dispatch
                          ...
                          never matching (#903). A signal nothing measured is **absent**,
                          and a comparison against an absent signal never matches: before
                          #1021 an absent number read as 0, so `graph.blast_radius < 5`
...
# Verify an answer by applying it: `hyctl edit` validates the file it just wrote.
# `hyctl dispatch --verify` is refused, a dispatch writes nothing, so its verifier
# would judge the repository rather than the answer (#982).
hyctl edit --file internal/auth/token.go "rotate signing key"
...
hyctl vet --confidence 0.8                 # as many Heads per file as its blast radius warrants
hyctl vet --dry-run                        # what would be reviewed, nothing dispatched
hyctl vet --local --json                   # local Heads only, machine-readable

# Expose the router as an OpenAI-compatible endpoint. The model field routes:
# `hydra`, `hydra/<enum>`, `hydra/t<N>`, or a head id. Loopback unless --token.
hyctl serve ; hyctl serve --enum SIMPLE --local
hyctl serve --addr 0.0.0.0:8788 --token "$HYDRA_SERVE_TOKEN"
OCR_LLM_URL=http://127.0.0.1:8787/v1 OCR_LLM_MODEL=hydra ocr review   # any OpenAI client
...
# Whether a prompt classifier could be fitted: vectors per embedding model, never summed
hyctl eval training ; hyctl eval training --json
# Does similarity to past work predict whether an enum passes? Leave-one-out,
# scored against a permutation null, so resolution from noise is not a finding.
hyctl eval classify ; hyctl eval classify --json
...
| `internal/swarm` | Fan-out dispatch: race / best (LLM judge) / all (CapScore rank). One `TierSelector` for both numeric and named hints, resolving through `dispatch.ResolveTier`; a second, config-driven selector meant `--tier simple` fanned out over a different head set than the same flag routed a single dispatch to (#782). `Options.tier` is the single reading of what a run routes on, `--tier` when given and otherwise the tier `--enum` resolves to: `Options.Enum` did not exist, so `--enum SIMPLE --swarm` fell through to `CapScoreSelector`'s top-N, which is the *most expensive* heads on the machine and the opposite of what the key asked for. Both the selector choice and the enum on the cost row read it, or a row records a routing key that did not route (#832). `judgeOptions` is the one derivation of what either judge may do, and it carries the run's `LocalOnly` and cost ceiling: both judges built their options from scratch and kept only the tier hint, so a `--local` run sent the whole prompt and both candidate answers to whichever head ranked highest. Measured before the fix, `hyctl vet --local --confidence` kept all three reviews local and made ten judge calls to a non-local head. #500 is the same defect in the sampling path, and the judges are what that pass did not cover. Both judges dispatch through a `dispatchFunc` rather than calling the dispatcher directly, because asserting on `judgeOptions` alone leaves either call site free to rebuild the options by hand, which is the defect and not a variant of it (#996). |
...
| `internal/capabilities` | Model capability scores: embedded `data.json` ⊕ runtime user overlay (`~/.hydra/models.json`) merged at discovery, so new models are added without a rebuild. `Score` reads that merged index; `ScoreLocal` is what every local model server's heads are scored by, and it reads the index **first**, falling back to the `ollamaFamilies` substring patterns. The local paths matched patterns only and never consulted the index, so the overlay reached every head except a local one: `hyctl models list` reported the user's number while `hyctl probe`, ranking and tiering used the pattern's, and the AI-BOM credited the curated catalog for a score the user chose. Keyed on the head id (`ollama/qwen3:8b`) and never also on the bare model name, since two spellings would disagree about whether the model was discovered; no built-in entry is shaped that way, so only the overlay can reach it (#989). Drives `hyctl models list\|add\|remove\|sync`. |
...
| `internal/serve` | Serves `POST /v1/chat/completions` and `GET /v1/models`, so every tool that speaks OpenAI reaches the router: policy, fallback, per-domain calibration and spend logging apply exactly as they do to `hyctl dispatch`. The client's `model` field is the **routing instruction**, which turns any OpenAI client's model picker into Hydra's routing UI (`hydra`, `hydra/<enum>`, `hydra/t<N>`, or a head id), and `/v1/models` advertises all of them. `local` routes but records **no** enum, being an alias for GRUNT rather than an enum of its own, or a cost row would name a routing key that does not exist (#832). The whole conversation is flattened into the prompt the router classifies rather than only the last turn: an agent loop's earlier turns carry tool results, which for a code-review client are the user's own source, and both the ledger and the egress gate read that prompt. Binds `127.0.0.1` and **refuses** an off-loopback bind with no token rather than warning: #923 added a check that reports when another local model server answers off-loopback, because Ollama ships with no authentication and says nothing when bound to `0.0.0.0`, and Hydra serving that way by default would make that check describe a risk Hydra itself created. An unparseable address reads as exposed, the safe reading of something unverifiable. `stream: true` is refused rather than answered with a whole body, since the client is parsing SSE and would see a malformed stream instead of a message it can act on, and both `max_tokens` and `max_completion_tokens` are read because the latter is what open-code-review actually sends. The package deliberately does not import `dispatch`; `cmd/hydra`'s `serveRouter` is the seam, as with `internal/workflow` and `internal/vet`. Drives `hyctl serve`. |
...
| `internal/ground` | Checks an answer against the context its prompt carried, as a new evidence source for machinery that already exists rather than a new subsystem. Deliberately narrow: does the answer state a number, path, flag or symbol appearing nowhere in the prompt? That is extrinsic hallucination, a model contradicting ground truth sitting in its own context, and it is the part of the problem a check with no trained model can speak to. Everything else it declines, and declining is most of the design: an unfenced prompt yields no verdict (`ErrNoContext`) and neither does an answer stating nothing checkable (`ErrNoClaims`), because a purely prose answer is exactly what this cannot judge and reporting it as verified would turn its weakest case into its most confident output. `util.Unwrap` is what finds the context, verifying each fence against the digest `WrapUntrusted` put in it, so a fence someone typed into a prompt is a claim rather than evidence. Support is the **whole prompt**, instructions included: a model repeating a flag the user named is grounded in what it was given. `claimKind` was narrowed by measurement, not taste: the first cut flagged 73% of grounded answers, and hyphenated prose (`best-effort`), acronyms read as camelCase (`PII`), lone short integers and `#`-prefixed citations were the four causes. It deliberately does **not** reuse `retrieve.Tokenize`, which splits camelCase and drops the joins: that is right for a search index and wrong here, where `internal/awsconf` matching `internal/executor` on the shared half is the exact failure being caught (#911). Measured over 690 pairs from this repository: 396 of 397 ungrounded answers flagged, including 184 of 184 single-identifier swaps; sensitivity 0.535 and a lower bound, since the positives are doc comments citing siblings and issues their own file does not contain. The verdict reaches `internal/trust` through `oracle.LLR` like any other, and `oracle.Measured` is what stops an unobserved source printing a prior as though it were measured, including #771's shape where positives accumulate and specificity stays pinned under 0.5. Not run automatically: a span score feeds `internal/reliability`, and an unmeasured check writing one would change what that number means without anyone choosing to. `hyctl oracle ground` (#907). |
| `internal/verify` | One resolution of "what checks this work": `go test ./...` inside a Go repo, else the `workspace.yaml` validator for the file's extension. Empty argv means nothing is configured to judge it, which a caller must report rather than treat as a pass. Lifted out of `internal/tui` so every caller agrees on what counts as a check (#843). It resolves the command for a **file on disk**, which is why it is the right tool for `hyctl edit` and the wrong one for a dispatch: `go test ./...` carries no placeholder and a dispatch writes nothing, so the candidate reached the verifier through neither argv nor the filesystem and the verdict described the repository. `hyctl dispatch --verify` is refused for that reason (#982). It is also what `cmd/hyverify` resolves with, shared rather than reimplemented, or the standalone tool and Hydra could disagree about whether work passed. That tool's own rule is #982 stated as a check: it refuses a candidate that is neither named in the verifier's argv nor inside the directory the verifier runs in, since a verdict is evidence about an answer only if the answer reached the thing the oracle inspects. |
| `internal/vet` | Reviews a diff and reports what is wrong with it. Alibaba's open-code-review (Apache-2.0) resolves which files are worth reviewing and by what rules through its **delegate** mode, which runs with no model, no API key and no spend; rewriting rule packs built from two years of internal defect data would be vanity, so Hydra uses them and owns everything above. The router picks a Head **per file**, not per rule group: a group can span a whole language, and one prompt holding every Go file in a branch both blows the context and makes a reported line number unattributable. `Options.Domain` is derived per file, so the Head measured best at Go on this machine is the one asked about a `.go` file. The package deliberately does not import `dispatch`, so it is testable without a model; `cmd/hydra`'s `vetRouter` is where the two meet, the same seam `internal/workflow` uses. Four silences refused, all the same failure: an unreadable reply is recorded as unparsed rather than as a clean file, a finding naming a file the Head was never shown is discarded **and counted** (an inventive Head is a fact worth surfacing), a file no rule matched is not reviewed at all, and a delegate `schema_version` the parser was not written against is refused, because a silent mis-parse reads as "no findings" and that is the one wrong answer a reviewer must never give. A range diffs from the **merge base**, not from the tip of `--from`, or a branch review reports undoing whatever the base branch did after the fork; a root commit diffs against the empty tree, since `sha^` does not resolve and its diff would otherwise come back empty. The commit message is passed as review background, fenced and labelled as a claim rather than an instruction, since in a review its author is the party under scrutiny. The exit code is computed after both renderings rather than inside one: `--json` returned before the check, so the mode a CI gate is likeliest to use was the mode that exited 0 on a blocking finding. `--confidence` swaps the single dispatch for the SPRT ensemble, with each file's target read off its blast radius through `trust.DefectModel.RequiredConfidence`: the flag's value is a floor the radius only raises. Three refusals keep it honest. A radius not read off a graph is reported as a **default, not a measurement**, the #251 lesson, said once before any spending rather than per file. A file that never reaches its target is marked `short of`, because its findings are still real while its confidence is not, and rendering the two alike would claim a confidence the run never reached. And an uncalibrated domain refuses **identically for every file**, so it is marked fatal and stops the run with one message naming a Head the router would actually have sampled (#835), instead of the same refusal charged once per file. `ShortOfBar` is defined as `Set() && !Cleared()` rather than by repeating the comparison, because spelled out as `Confidence < Target` the `Set()` guard is unreachable and a test cannot tell whether it is load-bearing. Agreement is counted **per finding, not per file**: the ensemble already produces every Head's answer and only the winner's was read, so the agreement between them was measured and thrown away, the same shape as #950. A file's single confidence is a statement about the whole answer text, so attaching it to a claim three of four Heads made and to one only one of them made says the same thing about both. Two findings are the same claim when they name the same file, line and severity; nothing fuzzy, because a line window needs a threshold and there is no honest number for one, which is why `internal/cache` refused an overlap ratio. The consequence is printed rather than hidden: the same defect a line away, or worded differently, is its own claim, so the count is a **lower bound**. Severity is part of the claim, since one Head calling something blocking while another calls it a nit is disagreement. A Head whose reply would not parse agrees with nothing but is still a voter, or every count on the file inflates; a Head that **failed** is not a voter at all, since being unable to answer is not the same as answering and not agreeing. Nothing is rendered for a single Head, where "1 of 1" would dress one opinion as a consensus, and what blocks is unchanged, since gating on agreement is a different decision from measuring it (#981). An ensembled file's run is written to the trust log with its LLR ledger, the same `logTrustRun` the `--confidence` dispatch path uses, and the report names the `hyctl trust outcome <task_hash>` that replays it. Without that write the feature is inert: the hash would name a run nothing recorded. Replaying a review beats `hyctl trust record` because it trains the **dissenters**, and a Head that disagreed with a finding later judged wrong is the only observation that moves specificity (#771). Nothing is trained from the ensemble's own answer, which would be circular and is the shape that leaves specificity on its bare prior. Drives `hyctl vet`. |
...
| `internal/classify` | What the verified corpus says about work like this, and it exists because the obvious version of #901's measured enum is circular: a model fitted on the enum the caller *declared* learns to reproduce the guess, and automating a guess is not measuring it. What the corpus holds that is not a guess is `Passed`, so the question asked is not "which enum would the caller have picked" but "for work like this, which enums have actually been passing". A neighbourhood is the k nearest examples by cosine **within one embedding model's space**, since two models are two spaces (#1004); per enum it reports the pass rate as a Beta posterior under the Laplace prior `internal/trust` already uses, so one pass out of one reads as 0.67 rather than certainty, and an enum under `MinNeighbours` is declined rather than reported as a rate. `DefaultK` is 10 from a sweep over 141 real commit subjects: accuracy plateaus from 3 to 10 and falls away above, and 10 is the top of the plateau rather than the sweep's argmax, which on 141 samples would be overfitting the sweep. The neighbourhood gate is a **quantile of the corpus's own similarity distribution**, not an absolute cosine, so it means the same on a corpus of any subject; it sits at p90 because the median is inert, measured against 120 commit subjects from an unrelated Go repository it refused **none** of them while keeping every real one. At p90 it keeps 95% of real work and refuses 52% of that unrelated work, which is stated rather than hidden: near-neighbour software prose genuinely overlaps, and roughly half still gets through. `Evaluate` is leave-one-out, deliberately asking a question that needs no counterfactual: "would another enum have done better" is unidentifiable and `internal/ope` already refuses that class, while "does similarity predict whether this enum passes" is answerable from what is recorded and is the assumption everything else rests on. It scores through `internal/reliability`, and **resolution is compared against a permutation null**, the highest any of 19 outcome shuffles reaches over the same neighbourhoods, which is significance at 1/(n+1). Resolution alone is not evidence: 120 coin flips produced 0.036 of it against an uncertainty of 0.25. Comparing against the *mean* of the shuffles is not a test either, since a single draw beats the mean of several about half the time. A rule can route on it through `corpus.known`/`corpus.pass_rate`/`corpus.support`, derived **only when a rule names one**, since each costs an embedding call and a pass over the corpus, and `signals.yaml` ships empty so the default machine routes byte-identically (#1020). Whether the estimate is worth acting on is still what `hyctl eval classify` answers, and on a corpus with no verdicts the honest answer is not yet. Drives `hyctl eval classify`. |
| `internal/evalset` | Oracle-verified examples: task, candidate, and ground truth from `internal/oracle`. The only trace-adjacent data kept verbatim and forever, because it is the only corpus the router can be improved against. Lives at `~/.hydra/evalset/`, deliberately outside `logs/` so nothing that prunes logs can reach it. `Add` writes a self-ignoring `.gitignore` into whatever directory holds a corpus, because `hyverify`'s default puts one *inside* the repository it verifies and an example carries the whole text of the file it judged, so `git add -A` committed the user's own source back to them, PII-marked records included. Restored whenever absent rather than only at creation, since sharing a corpus is meant to go through an explicit opt-in and a separate redacted one, never this file (#1011). Deduplicated on `(task_hash, candidate_hash)`, where the task hash is `TaskHashFor(prompt)` when the writer knows the task and `domain\x00source` when nobody named it. Only the first tells two tasks in one domain apart, so before #973 every Go answer verified by one command shared a task identity and dedup silently degraded to the candidate alone. The fallback keeps domain and source in the key deliberately: dropping them would make one answer in two languages a single example, and would re-add every record in an existing corpus on upgrade. PII is marked rather than dropped, since dropping it would bias the corpus. It is the user's own code and prompts, so **no Hydra package reachable from it may import `net`, `net/...` or `os/exec`** (`net/url` parses without sending, the one allowance), and its third-party surface is pinned by name so arriving at a socket through a new dependency is a decision rather than a side effect. The rule is on the packages **we write**, not the transitive set: `internal/util`'s file lock reaches `golang.org/x/sys/windows`, which imports `net` for its syscall bindings, so a transitive rule is false on Windows while saying nothing about whether we added an uploader. Until #844 it was a comment naming an export path that had no implementation, so it guarded nothing. A contribution path needs an explicit opt-in and a separate redacted corpus, never this file. That dedup check read the **whole corpus** per append, unmarshalling every stored candidate (a source file) to compare two hashes, so filling the store was quadratic: 11.5 ms per `Add` at 1,000 examples, 51.9 ms at 5,000, ~1 s and ~14 h cumulative at 100,000 (#796). A sidecar `examples.jsonl.idx` now holds just the two hashes, with a **fixed-width header recording the corpus byte size** so currency is one `os.Stat` rather than a rescan, exact because the corpus is append-only. Measured 43x at 5,000 and near-flat (0.48 ms empty → 1.21 ms at 5,000), per-record cost 10.3 us → ~145 ns. The sidecar is derived data and the corpus is always the authority: absent, malformed, or size-mismatched, it is rebuilt, so a crash between the corpus append and the header commit self-heals. The header is written **last** for that reason, it is the commit point. Writes take a store-wide `util.Lock`, like `internal/payload`, or two `hyctl` processes appending at once can leave corpus and sidecar disagreeing. Each example also records the routing decision it judges (`Enum`, `Tier`, `Head`), stored rather than re-derived because `routing.yaml` is editable and the map in force when the head ran is not recoverable later; `hyctl oracle verify` takes `--enum`/`--tier` and reads both off the span when `--span` names one, `hyctl edit` and `hyctl parallel`'s edit tasks are the writers that fill it: each atomically writes the candidate **first**, then runs the extension validator against that file, so the verdict is about the content just written and is real ground truth. Both record through one exported function, `editor.RecordVerifiedEdit`, rather than a second copy: `parallel` keeps its own edit mechanics by design but had drifted to recording nothing at all, not even the calibration outcome (#999). A rejected edit is filed too, before the rollback discards the content it judges, because a corpus of only passes cannot rank one head against another. An extension with no configured validator files nothing: `validatorPassed` is initialised true and stays true having checked nothing, and filing that would be the same mislabelling (#986). By contrast `hyctl dispatch --verify` deliberately files **nothing**: its verifier is the repo's own suite and a dispatch applies nothing to disk, so the candidate never reaches the verifier and the verdict is true of the working tree rather than of the answer. Filing it labelled the corpus with an unrelated outcome, which is worse than an empty corpus because the router would be fitted on it (#982, proven by inspecting the argv the verifier received). Ground truth needs the answer applied first, so corpus volume has to come from `hyctl edit`, not from a dispatch. `Readiness` gates fitting on a **per (enum, head)** floor, `MinObservationsPerHead` (25) on `MinComparableHeads` (2): measured on RouterEval (38 classes x 3811 models) and CodeRouterBench, a fitted table below ~10 observations per class loses to the strongest single head outright, by up to 16 points, and 25 is the first count whose gain interval excludes zero. Volume on one head is not evidence about another, so it never counts toward readiness, and an example with no head or no enum is kept but can never make an enum fittable. An example also carries the **task's embedding and the model that produced it**, which is what makes the corpus trainable at all: `TaskHash` is a hash, designed so two similar prompts land nowhere near each other, and #901's measured-enum phase needs the opposite. The vector is of the *instruction*, not the prompt the head saw, which is mostly unreviewed file content, and it is normalised through `cache.Normalize` because a vector is comparable only to one made the same way. Both fields or neither: two models put one sentence in different spaces and a `[]float32` names neither, so a vector kept without its model is a cosine against an incompatible basis that still returns a number. The package cannot compute it, `internal/embed` speaks HTTP and the no-egress rule above forbids that here, so the caller embeds and passes it in. `Trainable` reports per embedding model and never sums across them, since two models are two corpora, and flags a second vector length under one model name rather than filtering it, because cosine between two lengths is undefined rather than merely worse. Drives `hyctl eval list\|stats\|readiness\|training`. |
| `internal/cache` | The opt-in answer cache, and the only component in the tree that can make a dispatch return something no head just produced. That asymmetry sets the whole design: every other routing mistake degrades to a worse head, this one degrades to a confident answer to a question nobody asked, so the package refuses far more than it serves. `Normalize` is the single derivation of what a prompt is hashed, tokenized and embedded as (redaction included, so a secret cannot reach the store even through a caller that forgot to check), and an exact `Key` match is the only hit served without an argument. A near match's candidate is found by `sameQuestion` and then, where both sides carry a vector, has to clear a cosine threshold as well. That order is the point: the content tokens need no model, so the near-match path works on a machine with no embedder instead of leaving it an exact hash map, and the dense half is an upgrade on the candidate rather than a precondition for having one. It earns no refusal it does not already get, measured: over 79,800 pairs that must all be refused the token gate refuses every one and cosine alone still serves 79 at 0.85. Requiring a vector cost every near match on a machine without one, and on a machine with one it still vetoes 21 of 400 filler-word restatements that keep every content word in order. Not the uppercase ones: those 31 fail the token gate and should, since `retrieve.Tokenize` emits a camelCase identifier whole and split, so `LiteLLM` is `[litellm lite llm]` and `LITELLM` is `[litellm]`, and the words really did change. A refusal is counted for any of the three gates, since each is the cache declining rather than not holding the answer, and a prompt nothing resembled stays a miss (#1015). Whether the same words in another order were refused is answered lexically, by `sameWordsLocked`, because the dense test cannot run without a vector: the counter that says whether the order gate earns its place read zero on exactly the machines where that gate is the only one there is (#1025). `sameQuestion` requires the two prompts' content tokens to be identical **and in the same order**, function words aside, and `sameReferents` requires them to refer to the same things. That second gate is not a refinement of the first: a possessive fails "a word a prompt can differ in without changing what it asks" outright, since the referent *is* the question, so `my`, `your`, `their`, `this` and `that` sitting in the stopword list made "review my changes" and "review your changes" one question. All 12 swaps measured passed the content gate, and a real dispatch served 7 of 12 with an embedder and 12 of 12 without one, because with no vector there is no cosine veto and the gate is the only thing there is. Compared per class, so `my` is weighed against `your` and not against `this`, and the two kinds of referent are weighed differently. A **determiner** (`my`, `their`, `this`, `that`) says which thing, so the classes must match exactly: "rotate the signing key" is not "rotate their signing key". A **pronoun** (`i`, `you`, `me`, `them`) says who is asking, which a restatement drops and adds freely, so containment is enough and only a swap refuses. Both halves were found by measurement rather than chosen: strict everywhere refused the harness's own "please can you … for me" wrapper at 354 of 400, and lenient everywhere served "rotate their signing key" from "rotate the signing key" in a real dispatch. The strict half costs one real restatement, "the blast radius of this file" against "…of the file", which is the conservative direction on purpose, since which file `this` names is exactly the context a cache does not have. The cosine veto stays, because it is the only gate against a modal swap, `can I deploy this` against `should I deploy this`, where every differing word is a stopword and cosine refuses 4 of 5; tense is the class neither gate covers, `is the build green` against `was the build green` at 0.9687, recorded as a known limit rather than fixed on no measurement (#1035). Order carries the direction of an operation, and comparing sets discarded it: "merge develop into main" and "merge main into develop" were one question, and `Store.Lookup` served the first's answer for the second. The dense half does not cover it, six order-swapped pairs measure 0.9736 to 0.9913 cosine against a 0.95 threshold, so it rates a reversal as more alike than an average restatement and raising the threshold makes it worse. Free to fix, since every perturbation the false-hit harness counts as a restatement leaves the order alone: case, punctuation and whitespace are normalised before tokenizing and filler words are stopwords. Repeats are kept for the same reason, `test the test` is not `test` (#1010). That second gate exists because of a measurement, not a hunch: `--max-cost` against `--max-heads` sits at 0.8557 cosine and `internal/awsconf` against `internal/executor` at 0.8598, both above the 0.85 vLLM SR's own cache serves at (#911). Over 400 real commit subjects, 79,800 pairs every one of which must be refused, cosine alone serves 79 at 0.85 and 1 at 0.95 while the conjunction serves none at any threshold, and restatements differing only in whitespace or filler words are still served. Set equality rather than an overlap ratio because a ratio needs a threshold and there is no honest number for one: on a six-word question a single changed noun still scores 0.71. `Lookup` is inert and `Record` is what counts, so `--dry-run` can ask what would happen without the report afterwards claiming it did, and a dispatch the cache was never allowed to answer is not a miss it can be blamed for. A hit writes **no cost row**: it has no head and so no routing propensity, and `internal/ope` weights every row by one, so an invented propensity would corrupt every counterfactual computed afterwards (#605). It is `runlog.KindCacheHit` instead. Bounded by bytes and entries, oldest first. `hyctl trace cache` (#906). |
...
| `internal/ope` | Off-policy estimation. `SelfNormalized` recovers a population rate from a non-uniformly sampled log by weighting each row by the inverse of its inclusion probability. `Evaluate` answers the counterfactual, what a routing policy you did not run would have cost, as self-normalized IPS with a **percentile bootstrap interval**, never a bare point estimate. It refuses rather than answering when the logged policy had no chance of doing what the candidate would do: that question is *unidentifiable*, not merely uncertain, and a wide interval still gets read as a number. `ErrNoPropensity` is kept distinct from `ErrInsufficientSupport` because the remedies differ, raising `explore_rate` creates overlap for future rows but cannot fix rows already written. Effective sample size (Kish's (Σw)²/Σw²) is the honest count: a thousand rows dominated by one weight are worth about one observation, and the estimate is refused below `MinESS`. Weights are clipped by default and `Method` says so, since clipping is a bias-variance trade the reader has to be told about. Bootstrap seeded so the same log gives the same interval twice. Exists because averaging a sampled log inverted the true ranking of two heads in simulation, which would then change routing (#605). `Constraint` is the cheapest-measured-competent router as a counterfactual policy, asking the live machine which head it would choose for a row's domain; a row carrying no domain **abstains** rather than scoring 0, since "the policy would have routed elsewhere" and "this row cannot say" are different claims. Its estimate reads old contexts against today's calibration, which is the honest limit of it and is stated rather than hidden. |
| `internal/tui` | Bubble Tea TUI: init wizard, install flow |
| `internal/review` | Code review subcommand: approve, reject, diff, summary, qa over the files Hydra last edited. An approve or reject is the strongest ground truth Hydra gets, stronger than the editor's own syntax check, and `recordReviewOutcome` is what carries it to calibration, which needs the head that wrote the file. `headIDForFile` reads **both** edit logs for that: `last_edit.json` has a single slot and `hyctl parallel` never writes it, so reading only that one credited no head to every file a batch touched and a human's verdict trained nothing, #999's shape in the half #1001 did not cover. Measured on one machine: approving a `hyctl edit` file moved the cell from n=2 to n=3, approving a `hyctl parallel` file left it at n=1. The newer log is consulted first, since one file can appear in both and the run that wrote it last is the one whose head produced what is on disk, and each reader matches on the path, so a file in neither yields no head rather than someone else's. `SummaryResult` carries it too: Hydra had recorded the head all along and no review surface showed it, so the reviewer deciding whether to approve could not see which head produced the change (#1032). |
| `internal/editor` | Editor integration. The file being edited is embedded in the prompt through `util.WrapUntrusted`, labelled with its path, like every other path that hands a model externally-sourced content. This is the command whose prompt is *mostly* unreviewed file content and it was the only one framing it in prose alone. The nonce is derived from the content, so a fence typed into the file cannot impersonate the real one, which prose cannot prevent; and `util.Unwrap` can find the span again, which is how `internal/ground` locates the context it checks an answer against. Without a fence it yields no verdict at all, so the command most likely to benefit from "did the model invent a symbol that is not in this file" was the one the check could not see. The output markers delimit only the answer now; they used to fence the input too, so the model was told to write between the same markers it was reading between. `internal/dispatch`'s echo exemption is unaffected: `hiddenContextFor` lists only `--system` and the a2a handoff, so the file content was never a candidate (#947). `Result.ValidatorPassed` is a `*bool`, nil meaning no validator ran, because a plain bool initialised true reported "not checked" exactly like "checked and passed", and `internal/parallel` hardcoded the same claim on its success path. That mattered most for Go: `registry/workspace.yaml` shipped no `go` entry, so the language Hydra is written in was the one `hyctl edit` never validated and never filed a corpus example for, while `TestValidatorFor` asserted a `go` validator against a map it wrote itself (#998). An edit acts on `ResolveLink(req.File)`, because `os.Rename` replaces a symlink rather than following it while `os.ReadFile` follows it: editing one deleted the link, wrote a copy of its target's content under the link's name, left the file the caller named untouched and reported all of it as a clean success. Resolved once, right after the scope check, which is what keeps the snapshot, the backup, the write, the validator, the rollback, the diff and the reported path naming one file. **Only the final component, and only when it really is a link**: a rename through a symlinked directory already lands in the real one, and canonicalising every path instead respells ordinary edits on the platforms that have a spelling to expand, Windows 8.3 and macOS `/var`, which orphans whatever was keyed on the path the caller passed. `internal/parallel` calls the same exported helper, the seam `RecordVerifiedEdit` already uses, since it keeps its own edit mechanics (#1023). |
| `internal/workspace` | The write boundary: `Check`/`CheckRooted`/`Resolve` decide whether Hydra may touch a file at all, and every edit path (`hyctl edit`, `hyctl parallel`, `hyctl review`) asks through them. Containment is `filepath.Rel`, not a string prefix, which is what refuses `..` traversal and a sibling directory named `<root>-evil`. Both operands are resolved through symlinks first, because a lexical check judges how a path is spelled rather than where it points, and that broke the boundary in both directions: a symlinked directory inside a workspace wrote outside it, and a symlink to a `.env` walked past `defaultDeniedGlobs`, read the file the deny list exists to protect and put its contents in the prompt. The reverse cost nothing but the feature, a root or a file spelled through a different symlink was refused as "no workspace contains" while sitting inside it, which is what the test suite's own `sameDir` helper had been normalising away. Resolution walks up to the deepest existing ancestor and re-attaches the rest, since `hyctl edit` creates files and the directory a write lands in is what decides. Only the verdict moves: a permitted write still opens the path the caller named. `Root` stays as written and the refusal names both spellings, or a denial reads as wrong to the operator who typed one of them (#1019). |
